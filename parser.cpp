...
parser::parser(ifstream &_src, istream &_is, ostream &_os) :src(_src), is(_is), os(_os)
{
...
void parser::read_in()
{
	...
	vector<string>ph1_vec, ph2_vec, ph3_vec;
	...
	for (int i = 0; i < ins_cnt; ++i)
	{
		...
		if (name == "li" || name == "move") ptr = new assign(ph1, ph2);
...
void parser::instruction_fetch()
{
	plat[0] = NULL;
	if (ins_top >= ins_vec_sz)
	{
		return;
	}
